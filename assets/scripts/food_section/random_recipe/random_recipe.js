...
const randomRecipeInfo = document.querySelector(
...
let showCardsRandomRecipe = () => {
  const recipeSearchTag = document.querySelector(
    ".random-recipe-section__input"
  ).value;
  const recipeImgAmount = document.querySelector("#img-amount").value;
  if (recipeSearchTag !== "") {
    fetch(
      `https://api.spoonacular.com/recipes/findByIngredients?ingredients=${recipeSearchTag}&number=${recipeImgAmount}&apiKey=5f32bda9da1044e5aef1e657650e5b7b`
    )
      ...
      .then((cards) => {
        ...
        try {
          cards.forEach((card, i) => {
            ...
            fetch(
              `https://api.spoonacular.com/recipes/${recipeId}/information?includeNutrition=false&apiKey=5f32bda9da1044e5aef1e657650e5b7b`
            )
              .then((response) => response.json())
              .then((recipeData) => {
                try {
                ...
                <h4 href="${
                  card.title
                }" class="random-recipe-section__subtitle">${card.title}</h4>
                <img src="${card.image}" alt="${
                    i + 1
                  } картинка" class="random-recipe-section__img"><br/>
              ...
              .catch((error) => {
                console.log(error);
                randomRecipeContainer.innerHTML =
                  "Проверьте подключение и попробуйте повторить запрос.";
              });
          });
          if (cards.length === 0) {
            throw new ReferenceError(
              "Попробуйте изменить запрос: такой тег не найден. Поиск доступен только на английском языке."
            );
          }
          if (cards === undefined) {
            throw new SyntaxError(
      ...
      .then((randomRecipe) => {
        ...
        try {
          ...
          if (randomRecipe.length === 0) {
            throw new ReferenceError(
              "Что-то пошло не так. Попробуйте найти рецепты самостоятельно. Введите тег для поиска."
            );
          }
...
document
  .querySelector(".random-recipe-section__button")
  .addEventListener("click", showCardsRandomRecipe);
document
...
document
  .querySelector(".random-recipe-section__question-img")
  .addEventListener("click", () => {
    randomRecipeInfo.classList.remove("hidden");
  });

document
  .getElementById("randomRecipeInfoCloseButton")
  .addEventListener("click", () => {
