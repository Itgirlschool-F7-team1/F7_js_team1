...
const enterRecipeBlock = document.querySelector(
  ".recipe-section__enter-recipe"
);
const recipesBlock = document.querySelector(".recipe-section__recipes");
const enterRecipeCloseButton = document.querySelector(
  "#enterRecipeCloseButton"
);
...
let showRecipes = (Array) => {
  if (localStorage.getItem("enteredRecipes").length === 2) {
    cards.innerHTML =
      "Здесь будут ваши рецепты. Пожалуйста, добавьте первый рецепт.";
...
let showFullCard = (event) => {
  ...
  if (target.classList.contains("card-active")) {
    ...
    if (
      target.classList.value === "recipe-section__card" ||
      target.classList.value === "card__img" ||
      target.classList.value === "card__subtitle"
    ) {
    ...
    for (let i = 0; i < arrayToShow.length; i++) {
      ...
      if (
        target.id === cardArray[i].id ||
        targetParent.id === cardArray[i].id
      ) {
...
let filterRecipes = (event) => {
  ...
  newArray = recipes.filter((recipeItem) => {
    if (
      recipeItem.category === target.id ||
      recipeItem.category + "Alt" === target.id
    ) {
      return recipeItem;
    }
  });
...
//сохраняем данные
document
  .getElementById("enterRecipeSaveButton")
...
document.querySelector(".tags__list").addEventListener("click", function () {
  ...
  if (!enterRecipeBlock.classList.contains("hidden")) {
    enterRecipeBlock.classList.add("hidden");
    document
      .querySelector(".recipe-section__recipes")
      .classList.remove("hidden");
  }
});
document
  .querySelector(".tags__list_alternative")
