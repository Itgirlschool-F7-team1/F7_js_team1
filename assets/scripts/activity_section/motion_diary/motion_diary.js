...
let saveInfoSteps = () => {
  ...
  if (stepsToday === "") {
    document.querySelector("#errorMessage_dateMd").innerHTML =
      "Error! Введите, пожалуйста, дату.";
  } else {
    document.querySelector("#errorMessage_dateMd").innerHTML = "";
  }
  if (steps === "") {
    document.querySelector("#errorMessage_stepsMd").innerHTML =
      "Error! Введите, пожалуйста, число.";
  } else {
    document.querySelector("#errorMessage_stepsMd").innerHTML = "";
  }
  ...
  if (steps <= 250) {
    ...
    document.querySelector("#personalRecords_box").innerHTML =
      "Дороги выложены, делай шаг увереннее, ведь двери все распахнуты для тех,кто чист намерениями.";
  } else if (steps > 3000 && steps <= 8000) {
    document.querySelector("#personalRecords_box").innerHTML =
      "Время всегда на шаг опережает нас — но мысли наши опережают время!";
    ...
    document.querySelector("#personalRecords_box").innerHTML =
      "...если получится, я сделаю следующий шаг, а потом посмотрю, куда меня это заведёт.";
  } else if (steps > 22000 && steps <= 26000) {
    document.querySelector("#personalRecords_box").innerHTML =
      "Мы знаем, кто мы есть, но не знаем, кем мы можем быть.";
  } else if (steps > 26000 && steps <= 30000) {
    document.querySelector("#personalRecords_box").innerHTML =
      "О, не лети так Жизнь!<br>…Слегка замедли шаг…<br>Мне важен каждый миг и маленький пустяк.";
...
let getArrayChartDate = () => {
  ...
  if (stepsArray.length > null) {
    arrDate = stepsArray.map(function (object) {
      return object.today;
    });
    return arrDate;
  }
...
let getArrayChartSteps = () => {
  let stepsArray = JSON.parse(localStorage.getItem("enteredSteps"));
  if (stepsArray.length > null) {
    arrSteps = stepsArray.map(function (object) {
      return object.steps;
    });
    return arrSteps;
  }
...
const myChartMotionDiary = document
  .querySelector("#myChartMotionDiary")
  .getContext("2d");
...
let averageValueCalculation = () => {
  ...
  for (i = 0; i < count; i++) sum += +arrStepsAll[i];

  let averageValue = sum / count;
  document.querySelector("#personalRecords_mean").innerHTML =
