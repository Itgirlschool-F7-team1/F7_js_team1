...
let calculateIMT = () => {
  ...
  if (userDate === "") {
    document.getElementById("errorMessage_date_IMT").innerHTML =
      "Поле не заполнено. Введите дату.";
  }
  ...
  if (
    ...
    if (IMT <= 16) {
      document.querySelector(".IMTinfo").innerHTML =
        "Ваш вес ниже нормы (выраженный дефицит массы тела)";
    }

    if (IMT > 16 && IMT <= 18.5) {
      document.querySelector(".IMTinfo").innerHTML =
        "Ваш вес ниже нормы (дефицит массы тела)";
    }
    ...
    if (IMT > 25 && IMT <= 30) {
      document.querySelector(".IMTinfo").innerHTML =
        "У Вас избыточная масса тела (предожирение)";
    }
    if (IMT > 30 && IMT <= 35) {
      document.querySelector(".IMTinfo").innerHTML =
        "У Вас избыточная масса тела (Ожирение 1 степени)";
    }
    if (IMT > 35 && IMT <= 40) {
      document.querySelector(".IMTinfo").innerHTML =
        "У Вас избыточная масса тела (Ожирение 2 степени)";
    }
    if (IMT > 40) {
      document.querySelector(".IMTinfo").innerHTML =
        "У Вас избыточная масса тела (Ожирение 3 степени)";
    }
  }
};
...
const ctx_historyIMT = document
  .getElementById("myChart_historyIMT")
  .getContext("2d");
